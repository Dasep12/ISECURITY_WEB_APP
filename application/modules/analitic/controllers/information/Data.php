...
class Data extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->isecdb = $this->load->database('isecurity', TRUE);
    }

    public function index()
    {
        $wl  = $this->isecdb->query("select wilayah , area_kerja , npk  from employee where id_employee='" . $this->session->userdata('id_akun') . "' ")->row();
        $wil  =  $wl->wilayah;


        $sql = 'SELECT b.nama,b.npk , b.`tanggal_lahir`  , e.`area_kerja` , e.`wilayah` , b.`gol_darah` , b.`ktp` , b.kk , b.`email` , b.`no_hp` ,b.`no_emergency` , b.`tinggi_badan` , b.`berat_badan` , b.`imt` , b.`keterangan` , b.`jl_ktp` , b.`rt_ktp` , b.`rw_ktp`,b.`kel_ktp` , b.`kec_ktp` , b.`kota_ktp` , b.`provinsi_ktp` , b.`rt_dom` , b.`rw_dom`   , b.`jl_dom` , b.`kel_dom` , b.`kec_dom` , b.`kota_dom` , b.`provinsi_dom` ,e.`no_kta` , e.`expired_kta` , e.`jabatan` , e.`status_kta`  , br.`foto` FROM `biodata` b ,`employee` e , `berkas` br   WHERE b.`id_biodata` = e.`id_employee`  AND e.`id_employee` = br.`id_berkas`  AND  (e.`jabatan` != "MANAGER" OR e.`jabatan` != "SUPERVISOR" OR e.`jabatan` != "PIC" OR e.jabatan != "ARH") AND b.`nama` != "SUPERADMIN" AND b.status = 1 and e.status = 1   ORDER BY b.`nama` ASC';

        $data = [
            'link'  => $this->uri->segment(2),
            'data'  => $this->isecdb->query("select wilayah , area_kerja , npk from employee where id_employee='" . $this->session->userdata('id_akun') . "' ")->row(),
            'anggota'   => $this->isecdb->query($sql)
        ];
        $this->template->load("template/analityc/template_information", "information/daftar_anggota", $data);
    }
    ...
    public function modalBiodata()
    {
        ...
        $data = [
            'data' => $this->isecdb->get_where("profiling", ['npk' => $npk])->row()
        ];
        $this->load->view("information/detail_modal_biodata", $data);
    }
    ...
    public function modalAbsensi(Type $var = null)
    {
        ...
        $this->load->view("information/load_absensi", $data);
    }
    ...
    public function unduh()
    {
        ...
        $export = $this->isecdb->query('SELECT b.nama,b.npk , b.`tanggal_lahir`  , e.`area_kerja` , e.`wilayah` , b.`gol_darah` , b.`ktp` , b.kk , b.`email` , b.`no_hp` ,
