...
class Dashboard_dev_2 extends CI_Controller
{
    public function __construct(Type $var = null)
    {
        parent::__construct();
        $this->load->model(['srs/M_dashboard_dev','Roles_m']);
    ...
    {
        $data_area = $this->M_dashboard_dev->area()->result();
        $risk = $this->M_dashboard_dev->risk()->result_array();
        $target_assets = $this->M_dashboard_dev->target_assets()->result_array();
        $grap_risk = $this->M_dashboard_dev->grap_risk()->result_array();
        // $res_risk_level_max = $this->M_dashboard_dev->grap_risk_level_max()->result_array();
        $grap_risk_source = $this->M_dashboard_dev->grap_risk_source()->result_array();
        $grap_trans_month = $this->M_dashboard_dev->grap_trans_month()->result();
        $grap_trans_area = $this->M_dashboard_dev->grap_trans_area()->result();
        $grap_trans = $this->M_dashboard_dev->grap_trans()->result();
        $grap_target_assets = $this->M_dashboard_dev->grap_target_assets()->result_array();
        $res_iso = $this->M_dashboard_dev->grap_srs()->result_array();
        $res_soi = $this->M_dashboard_dev->grap_soi()->result_array();
        $res_soi_avg_month = $this->M_dashboard_dev->grap_soi_avg_month()->result_array();
        $res_soi_avgpilar = $this->M_dashboard_dev->grap_soi_avgpilar()->result();
        $res_soi_montharea = $this->M_dashboard_dev->grap_soi_avg_montharea()->result_array();
        $res_type_source = $this->M_dashboard_dev->grap_type_source()->result_array();
        $current_year = date('Y');

        // echo '<pre>';
        // print_r(array_values($res_type_source[0]));
        // print_r($res_type_source);
        // die();
        ...
        $data = [
            ...
            'grap_type_source' => json_encode(array_values($res_type_source[0]), true)
        ];
        
        $this->template->load("template/analityc/template_srs", "srs/dashboard_dev_2", $data);
    }

    public function grap_internal_source()
    {
        $grap_risk_source = $this->M_dashboard_dev->grap_risk_source()->result_array();
    ...
    public function grap_srs()
    {
        ...
        // {
            $res_iso = $this->M_dashboard_dev->grap_srs()->result_array();
            $res_soi = $this->M_dashboard_dev->grap_soi()->result_array();
            $res_risk_source = $this->M_dashboard_dev->grap_risk_source()->result_array();
            $res_risk = $this->M_dashboard_dev->grap_risk()->result_array();
            // $res_risk_level_max = $this->M_dashboard_dev->grap_risk_level_max()->result_array();
            $res_target_assets = $this->M_dashboard_dev->grap_target_assets()->result_array();
            $res_trans_month = $this->M_dashboard_dev->grap_trans_month()->result_array();
            $res_trans_area = $this->M_dashboard_dev->grap_trans_area()->result_array();
            $res_trans = $this->M_dashboard_dev->grap_trans()->result_array();
            $res_soi_avg_month = $this->M_dashboard_dev->grap_soi_avg_month()->result_array();
            $res_soi_avgpilar = $this->M_dashboard_dev->grap_soi_avgpilar()->result();
            $res_soi_montharea = $this->M_dashboard_dev->grap_soi_avg_montharea()->result_array();
            $res_type_source = $this->M_dashboard_dev->grap_type_source()->result_array();
            ...
            $arr = array(
                ...
                'grap_soi_avgpilar' => $res_soi_avgpilar,
                'grap_soi_avg_areamonth' => $sorAvgArea,
                'grap_type_source' => array_values($res_type_source[0])
            );
    ...
    public function grap_detail_risk_source()
    {
        ...
        $res_riSo_detail = $this->M_dashboard_dev->grap_detail_risksource('risk_source_id')->result_array();
        // GRAP RISK SOURCE - MONTH //
        $res_riSo_month_sub1 = $this->M_dashboard_dev->grap_detail_risksource('risksource_sub1_id')->result_array();
        // GRAP RISK SOURCE SUB 1 - TOTAL //
        $res_riSosub1_detail = $this->M_dashboard_dev->grap_detail_risksource_sub('risksource_sub1_id')->result_array();
        // GRAP RISK SOURCE SUB 2 - TOTAL //
        $res_risksub2_detail = $this->M_dashboard_dev->grap_detail_risksource_sub('risksource_sub2_id')->result_array();
        // RISK SOURCE SUB 2 MONTH //
        $res_riSo_month_sub2 = $this->M_dashboard_dev->grap_detail_risksource('risksource_sub2_id')->result_array();
    ...
    {
        // Main assets data
        $res_assets_detail = $this->M_dashboard_dev->grap_detail_assets('assets_id')->result_array();
        // TARGET ASSETS SUB 1 MONTH
        $res_assets_sub1_month = $this->M_dashboard_dev->grap_detail_assets('assets_sub1_id')->result_array();
        // TARGET ASSETS SUB 1
        $res_assetssub1_detail = $this->M_dashboard_dev->grap_detail_assets_sub('assets_sub1_id')->result_array();
        // TARGET ASSETS SUB 2
        $res_assetssub2_detail = $this->M_dashboard_dev->grap_detail_assets_sub('assets_sub2_id')->result_array();
        // TARGET ASSETS SUB 2 MONTH
        $res_assets_sub2_month = $this->M_dashboard_dev->grap_detail_assets('assets_sub2_id')->result_array();
    ...
    {
        $res_risk_detail = $this->M_dashboard_dev->grap_detail_risk('risk_id')->result_array();
        $res_risk_sub1 = $this->M_dashboard_dev->grap_detail_risk_sub('risk_sub1_id')->result_array();
        $res_risk_sub1_month = $this->M_dashboard_dev->grap_detail_risk('risk_sub1_id')->result_array();
        $res_risk_sub2 = $this->M_dashboard_dev->grap_detail_risk_sub('risk_sub2_id')->result_array();
        $res_risk_sub2_month = $this->M_dashboard_dev->grap_detail_risk('risk_sub2_id')->result_array();
    ...
    {
        $data = $this->M_dashboard_dev->grap_trans_month()->result();
        $area = $this->M_dashboard_dev->grap_trans_area()->result();
        $risk_avg_month = $this->M_dashboard_dev->grap_risk_month_avg()->result_array();
        $risk_avg = $this->M_dashboard_dev->grap_risk_avg()->result_array();
        $grap_risk = $this->M_dashboard_dev->grap_risk()->result_array();
