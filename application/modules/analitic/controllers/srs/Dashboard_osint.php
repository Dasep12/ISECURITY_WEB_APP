...
class Dashboard_osint extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data = [
            ...
            'select_year_filter' => form_dropdown('year_filter', $opt_yea,  date('Y'), 'id="yearFilter" class="form-control" required'),
        ];
        $this->template->load("template/analityc/template_osint", "srs/dashboard_osint", $data);
    }
    ...
    public function getNegativeSentiment()
    {
        $year = "2023";
        $data = $this->models->getNegativeSentiment($year)->result();
        echo json_encode($data);
    }
    ...
    public function getFormat()
    {
        $year = $this->input->post("year");
        $month = $this->input->post("month");
        $data = $this->models->getFormat($year)->result();
        echo json_encode($data);
    }
