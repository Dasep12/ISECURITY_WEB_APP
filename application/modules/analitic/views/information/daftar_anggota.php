<div class="container p-3">
    ...
    <div class="card mb-5">
        ...
        <div class="card-body">
            <a class="btn btn-success btn-sm mb-4 " href="<?= base_url("SA/Profiling/unduh") ?>">Download</a>
            <div class="table-responsive">
                <table id="table_id" class="table-bordered table table-small">
                        ...
                        foreach ($anggota->result() as $agt) :  ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a data-backdrop="static" data-keyboard="false" data-toggle="modal" data-npk="<?= $agt->npk ?>" data-nama="<?= $agt->nama ?>" data-area="<?= $agt->area_kerja ?>" data-wilayah="<?= $agt->wilayah ?>" data-target="#absensi-data" title="Absensi" href="#" class=" ms-3"><i class="fa fa-clock"></i></a>
                                </td>
                            </tr>
                        <?php endforeach ?>
                    </tbody>
                ...
                <div class="row">
                    ...
                    <div class="col lg-6">
                        <form name="report_absensi" id="report_absen" action="#">
                            ...
                            <input type="hidden" name="wilayah_" id="wilayah_">
...
<script>
    $("#edit-data").on("show.bs.modal", function(event) {
        ...
        $.ajax({
            url: "<?= base_url('analitic/information/Data/modalBiodata') ?>",
            method: "POST",
            data: 'npk=' + npk,
            success: function(e) {
                console.log(npk)
    ...
    $("#absensi-data").on("show.bs.modal", function(event) {
        ...
        document.getElementById("wilayah_").value = wilayah;
    ...
    $("#report_absen").on('submit', function(e) {
        ...
        var wilayah = document.getElementById("wilayah_").value;
        var tahun = $("select[name=tahun_absensi] option:selected").val();
        var bulan = $("select[name=bulan_absensi] option:selected").val();
        $.ajax({
            url: "<?= base_url('analitic/information/Data/modalAbsensi') ?>",
            method: "POST",
            data: "npk=" + npk + "&bulan=" + bulan + "&tahun=" + tahun + "&wilayah=" + wilayah,
            beforeSend: function() {
