...
class M_internal_source extends CI_Model
{
    ...
    {
        $npk = user_npk();
        $this->srsdb->select('a.id, a.event_name, a.event_date, a.impact_level, a.status, asu.title area, ass.title assets, rss.title risk_source, ris.title risk');
    ...
    public function detail()
    {
        ...
        $q = $this->srsdb->query("
                SELECT a.id, a.event_name, a.event_date, a.no_urut, a.no_reference, a.reporter, a.chronology, vfi.level financial_level, vop.level operational_level, vre.level reputation_level, sdm.level sdm_level, a.impact_level, a.attach_file_1, a.attach_file_2, a.attach_file_3, a.attach_file_4, a.attach_file_5, asu.title area, asu1.title area_sub1, asu2.title area_sub2, asu3.title area_sub3, ass.title assets, ass1.title assets_sub1, ass2.title assets_sub2, rss.title risksource, rss1.title risksource1, rss2.title risksource2, ris.title risk, ris1.title risk1, ris2.title risk2
                    , (select name from isecurity.dbo.admisecsgp_mstusr where npk=a.created_by) author,
                    (select amu.name from isecurity.dbo.admisec_area_users aau 
                        inner join isecurity.dbo.admisecsgp_mstusr amu ON amu.npk=aau.npk 
                        inner join isecurity.dbo.admisecsgp_mstsite ams ON ams.site_id=aau.site_id 
                        inner join srs_bi.dbo.admiseciso_area_sub aas ON aas.wil_id=ams.id_wilayah where aas.id=a.area_id) section_head
    ...
    public function save()
    {
        ...
        $chronology = $this->input->post('chronology', true);
        ...
        if($_FILES['attach']['name'][0] !== '')
        {
            ...
            $field_file_implode = implode (", ", $field_file_arr);
        }
        else
        {
            $attach_file = '';
        ...
        $this->srsdb->query($column.$q_implode);
    ...
    public function update()
    {
        ...
        if($_FILES['attach']['name'][0] !== '')
        {
            ...
            $total_arr1 = count($no_attached_file);
            $arr1 = $no_attached_file;
            $arr3 = count($_FILES['attach']['name']);
            $arr2 = range(1,5) ;
            
            $missing = array_diff($arr2,$arr1);
            $no_attached_ready = array();
            foreach ($missing as $key){
                // if (count($key) < $total_arr1) {
                    $no_attached_ready[] = $key;
                // }
            }

            // if(count($no_attached_ready) > $total_arr1)
            $hasil = array_slice($no_attached_ready, 0, count($arr2) - $total_arr1);

            echo count($arr2) .'-'. $total_arr1.'= '.count($arr2) - $total_arr1;
            echo '<br>';
            var_dump($arr3);
            die();
            ...
            foreach ($missing as $no) {
                // $no_attached_ready[] = $key;
                    
                foreach ($upload as $key => $ifl) {
                    // if($ifl['file_name'] !== '')
                    // {
                        $column_file_arr[] = 'attach_file_'.$no;
                        $field_file_arr[] = "'".$ifl['file_name']."'";
                        // $field_file++;
                    // }
                }
    ...
    public function approve()
    {
        ...
        // {
            ...
            $this->srsdb->query("
                UPDATE admiseciso_transaction SET status=1 WHERE id='$id'
            ");
