...
class M_dashboard_humint extends CI_Model
{
    ...
    public function area()
    {
        ...
        $q = "SELECT id, title 
                ...
                WHERE area_categ_id='1' AND status=1 AND is_humint=1";
    ...
    public function grap_trend_soi_index()
    {
        $area = $this->input->post('area_fil', true);
        $year = $this->input->post('year_fil', true);
        $year = empty($year) ? date('Y') : $year;
    ...
    public function grap_top_index()
    {
        ...
        // $q = "
        //     SELECT rso.id, rso.title, ISNULL(count(1), 0) total,
        //         case 
        //             when rso.type_source is null 
        //             then (select type_source from srs_bi.dbo.admiseciso_risksource_sub where id=tis.risksource_sub1_id)
        //             else rso.type_source
        //             end as type_source
        //         FROM srs_bi.dbo.admiseciso_transaction tis
        //         LEFT JOIN srs_bi.dbo.admiseciso_risksource_sub rso ON rso.id=tis.risk_source_id
        //     WHERE tis.status=1 and tis.disable=0 and rso.risksource_categ_id=1 ";
        ...
        //     $q .= " GROUP BY rso.id, rso.title, tis.risksource_sub1_id, rso.type_source
        //     ORDER BY total DESC
        // ";
