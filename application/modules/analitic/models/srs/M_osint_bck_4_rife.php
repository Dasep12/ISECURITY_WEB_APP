...
class M_osint extends CI_Model
{
    ...
    public function getCategorySub1($id='')
    {
        $subId  = $id;
        if($_POST)
        {
    ...
    {
        return $this->osidb->query("SELECT ashd.sub_id  , arl.[level] , arl.id level_id , arl.description  , ashd.name  FROM admisecosint_sub_header_data ashd 
    ...
    public function save()
    {
        ...
        $impact = $this->input->post('impact_level', true);
        ...
        $vulnerabilityLevel = max($sdmSector, $reput);

        // echo '<pre>';
        // print_r($_POST);
        // var_dump($vulnerabilityLevel);
        // die();
    ...
    public function update()
    {
        ...
        $data = [
            ...
            'vulnerability_level_id' => $vulnerabilityLevel,
            'risk_level' => $LevelriskTreat,
    ...
    {
        $this->osidb->select('t.id, t.activity_name as act , asu.title as plant,ashd.name as media,ashd1.name as sub_media, ashd2.name as jenis_media, ashd3.name as risk,CAST(t.date as DATE) as event_date,t.impact_level , sng.name sentiment , t.status');
        ...
        $this->osidb->join('admisecosint_sub_header_data sng', 'sng.sub_id = t.hatespeech_type_id', 'left');
    ...
    public function get_edit()
    {
        ...
        $this->osidb->select('tr.id,tr.activity_name , tr.date , tr.area_id , tr.sub_area2_id , tr.sub_target_issue1_id , tr.target_issue_id , tr.sub_target_issue3_id , tr.sub_risk_source , tr.risk_source , tr.sub_risk1_source , tr.employe_plant , tr.sub_target_issue2_id , tr.sub_area1_id , tr.plant_id , tr.impact_level , tr.description , tr.sub_media_id , tr.url1 , tr.url2 , asu.title as plant , lev.level as sdm_level , lev2.level as reputasi_level , tr.created_by , ashd.name as media , tr.media_id , tr.media_level_id , tr.sub1_media_id , melvl.level media_level , ashd1.name as jenis_media , tr.regional_id , tr.regional_level_id , reglvl.level regional_level , tr.legalitas_id , tr.legalitas_sub1_id , tr.legalitas_level_id , leglvl.level legalitas_level , tr.format_id , tr.format_level_id , frmlvl.level format_level , tr.sdm_sector_level_id , tr.reputasi_level_id , tr.risk_level ,  tr.vulnerability_level_id, vurlvl.level vulnerability_level, tr.hatespeech_type_id');
        ...
        $this->osidb->join('admisecosint_risk_level as vurlvl', 'vurlvl.id=(select level_id from admisecosint_sub_header_data where sub_id=tr.vulnerability_level_id)', 'left');
