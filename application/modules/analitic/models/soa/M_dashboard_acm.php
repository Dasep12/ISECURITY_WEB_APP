...
class M_dashboard_acm extends CI_Model
{
    ...
    public function area()
    {
        $wil = user_wilayah();
        $npk = user_npk();
        $q = "SELECT id_lokasi AS id, lokasi AS title 
                    FROM MiddlewareACM.dbo.EmployeeToACM 
                ";
    ...
    public function employeeAcmTotal()
    {
        ...
        $q = "SELECT count(1) AS total
                FROM MiddlewareACM.dbo.EmployeeToACM eta
            WHERE eta.status=1";

        if(!empty($area)) $q .= " AND eta.id_lokasi='$area'";
