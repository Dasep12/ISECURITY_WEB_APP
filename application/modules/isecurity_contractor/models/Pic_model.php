...
class Pic_model extends CI_Model
{
    ...
    private function _get_datatables_query()
	{
        ...
        $as = $this->session->userdata('wilayah');
         $this->db->from($this->table);
        $this->db->where('wilayah',$as);
    ...
    {
        $query = $this->db->query("SELECT * FROM admviewtrans_sgpprofile  GROUP BY wilayah");
        return $query->result();
    }
    ...
    function CountKTAWil($wil,$kta)
    {
        $query = $this->db->query("SELECT status_kta , COUNT(status_kta) AS total FROM admviewtrans_sgpprofile  where status_kta =  '$kta' AND wilayah = '$wil' ");
        return $query->row();
    }

    function CountKTAArea($wil,$area)
    {
        $query = $this->db->query("SELECT status_kta , COUNT(status_kta) AS total FROM admviewtrans_sgpprofile  where status_kta =  '$kta' AND area_kerja = '$area' ");
        return $query->row();
    }

    function getAbsensi($id, $date)
    {
        return $this->db->query('SELECT * FROM  admviewtrans_sgpabsensi  WHERE id_biodata = "' . $id . '" AND in_date LIKE "%' .  $date . '%"');
    }
