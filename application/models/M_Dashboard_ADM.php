...
class M_Dashboard_ADM extends CI_Model
{

    public function getPlant($wil)
    {
        $res = $this->db->query("SELECT plant_name FROM admisecsgp_mstplant WHERE admisecsgp_mstsite_site_id ='" . $wil . "' order by plant_name ASC ")->result();
        return $res;
    }
    ...
    public function performancePatroliAllPlant($year, $plant, $site)
    {
        $all = count($this->getPlant($site));
        $result = array();
        for ($i = 1; $i <= 12; $i++) {
            $hariPembagi = cal_days_in_month(CAL_GREGORIAN, $i, $year);
            $pembagi = 3 * $hariPembagi;
    ...
    public function trendPatrolSetahun($year, $month, $plant)
    {
        ...
        if ($plant == "0") {
            $where .= "a.admisecsgp_mstsite_site_id = '" . $site . "' and month(ath.date_patroli)='" . $month . "'
    ...
    public function getTemuanPatroliAll($year, $site)
    {
        $data = array();
        for ($i = 1; $i <= 12; $i++) {
            $query = $this->queryTemuanAll($year, $i, $site);
            $data[] = $query->num_rows();
        }
        return $data;
    }

    private function queryTemuanAll($year, $month, $site)
    {
        ...
        where  pl.admisecsgp_mstsite_site_id = '" . $site . "' AND
    ...
    public function queryTemuanRegu($year, $month, $regu)
    {
        ...
        where pl.admisecsgp_mstsite_site_id ='" . $this->session->userdata("site_id") . "' AND rl.[level]  = 'Security'   and usr.status  = 1 and usr.patrol_group = '" . $regu . "'
